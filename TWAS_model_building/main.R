...
setwd("/path/to/work/folder")

"%&%" <- function(a,b) paste(a,b, sep = "")
prefix <- "CRC_Asian_model"
...
###### II. input files ######
snp_annot_file <- paste0("file_with_variantID_and_rsID")
gwas_file <- paste0("genotype_bim_file_from_Plink")
genotype_file <- paste0("genotype_txt_file_from_Plink")

gene_annot_file <- "gencode.v19.annotation.gtf"
expression <- "expression_PEER_inverse.txt"

###### III. set function  ######
snp_annot <- read.table(snp_annot_file, header = T, stringsAsFactors = F)
gwassnp <- read.table(gwas_file,head=FALSE)
colnames(gwassnp)[2] <- "varID"
snp_annot<-snp_annot[snp_annot$varID %in% gwassnp$varID,]
...
## need modification based on your files
...
get_maf_filtered_genotype <- function(genotype_file_name,  maf, samples) {
  gt_df<- fread(genotype_file,sep="\t",header=T)
  gt_df <- as.data.frame(gt_df)
  gt_df$IID <- gsub("WG\\d+-\\S+-(CNUHHCRC_\\d+)@\\d+","\\1",gt_df$IID, perl=TRUE)
  gt_df$IID <- gsub('CNUHH-CRC-(\\d+)_QC-3158-Cai_\\S+','CNUHHCRC_\\1',gt_df$IID, perl=T)
  gt_df$IID <- gsub('FMMU-(CC\\d+N)_QC-3158-Cai_\\S+','FMMU_\\1',gt_df$IID, perl =T)
  gt_df$IID <- gsub('(HCES2-\\d+)_QC-3158-Cai_\\S+','\\1', gt_df$IID, perl=T)
  row.names(gt_df) <- gt_df$IID
  gt_df1 <- gt_df[gt_df$IID %in% samples,7:dim(gt_df)[2]]
  effect_allele_freqs <- colMeans(gt_df1) / 2
  gt_df1 <- as.data.frame(gt_df1)
  gt_df1 <- gt_df1[,which((effect_allele_freqs >= maf) & (effect_allele_freqs <= 1-maf))]
  colnames(gt_df1) <- gsub("(\\S+)_\\S+","\\1",colnames(gt_df1),perl=TRUE)
...
###### IV. run analysis ######
    ...
    # output file
    model_summary_file <- './output-folder/' %&% prefix %&% '_chr' %&% chrom %&% '_model_summaries.txt'
    ...
    weights_file <- './output-folder/' %&% prefix %&% '_chr' %&% chrom %&% '_weights.txt'
    weights_col <- c('gene_id', 'rsid', 'varID', 'ref', 'alt', 'beta')
    write(weights_col, file = weights_file, ncol = 6, sep = '\t')
  
    covariance_file <- './output-folder/' %&% prefix %&% '_chr' %&% chrom %&% '_covariances.txt'
